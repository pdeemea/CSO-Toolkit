CREATE OR REPLACE FUNCTION public.__aag_fn_ts_init() RETURNS void AS
$BODY$
declare
    v_sql text;
    v_db_oid text;
begin
    ...
    v_sql := 'DROP EXTERNAL TABLE IF EXISTS public.__aag_db_files_ext';
    EXECUTE v_sql;
    v_sql := 'CREATE EXTERNAL WEB TABLE public.__aag_db_files_ext ' ||
...
create table public.__aag_db_files as
    SELECT  current_database() as dbname,
            n.nspname AS schemaname,
            c.relname AS tablename,
            c.relstorage,
            c.relkind,
            sum(db.size)/1000./1000. AS size_mb
        FROM ONLY public.__aag_db_files_ext db
            INNER JOIN pg_class c ON split_part(db.relfilenode, '.'::text, 1) = c.relfilenode::text
...
CREATE OR REPLACE FUNCTION public.__aag_fn_get_skew(out schema_name      varchar,
...
$$
DECLARE
    v_function_name text := '__aag_fn_get_skew';
    v_location int;
    v_sql text;
    v_db_oid text;
...
psql -d dsstest -c "copy (select * from public.__aag_fn_get_skew()) to stdout csv header delimiter '|';" > data/table_skew.csv
psql -d dssprod -c "copy (select * from public.__aag_fn_get_skew()) to stdout csv header delimiter '|';" > data/table_skew.csv
